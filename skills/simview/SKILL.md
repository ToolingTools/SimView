...
1. Call `list_devices` and select an available device. Prefer an explicitly
   supplied device ID; otherwise SimView selects the first available device and
   the user can switch devices in the preview. Always call `connect_device`
   ...
   use the connected session without opening the preview. `list_simulators`,
   `connect_simulator`, and iOS UDIDs remain compatibility aliases.
...
3. Use `tap_element` for semantic targets. It re-resolves the target and sends a
   platform-native tap. Use `wait_for_element` or observe again to verify outcome;
