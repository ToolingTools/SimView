...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-09 (commit 86fd077). Confidence: 98%.
...
- `native/SimViewCore/Sources/SimViewAXShim/SimViewAXShim.m` — 3 bug fixes, last fix yesterday; 11 commits/90d
- `packages/mcp/src/server.ts` — 3 bug fixes, last fix 4 days ago; 21 commits/90d
- `packages/app/src/index.tsx` — 3 bug fixes, last fix 9 days ago; 19 commits/90d
- `tests/mcp-tools.test.ts` — 2 bug fixes, last fix 4 days ago; 17 commits/90d
- `packages/mcp/src/session.ts` — 2 bug fixes, last fix 9 days ago; 15 commits/90d

### Code health
Three co-equal signals: defect risk 5.86/10 avg, hotspot health 4.69/10 (stable), worst `packages/cli/src/index.ts` at 1.0/10 · maintainability 8.48/10 · performance risk 39 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- `docs/architecture.md` — hidden coupling — impact −1.2
- `packages/client/src/protocol.ts` — churn risk — impact −1.1
- `packages/app/src/index.tsx` — change entropy — impact −1.0

### Standing decisions (ask `get_why` before diverging)
- XCTest is the primary iOS accessibility provider — This makes element discovery dependable for the observed app while preserving a usable degraded path
- Treat XCTest provenance as a first-class accessibility contract — Provider identity must remain visible in snapshots and compact output so callers can distinguish nat
- Native accessibility remains authoritative when Metro inspection is unavailable — Optional framework inspection must not make native interaction unavailable, leak raw Fiber or source
