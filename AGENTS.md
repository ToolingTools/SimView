...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-08 (commit 40153f3). Confidence: 98%.
...
- `native/SimViewCore/Sources/SimViewAXShim/SimViewAXShim.m` — 3 bug fixes, last fix today; 9 commits/90d
- `packages/mcp/src/server.ts` — 3 bug fixes, last fix 3 days ago; 18 commits/90d
- `packages/app/src/index.tsx` — 3 bug fixes, last fix 8 days ago; 18 commits/90d
- `tests/mcp-tools.test.ts` — 2 bug fixes, last fix 3 days ago; 14 commits/90d
- `packages/mcp/src/session.ts` — 2 bug fixes, last fix 8 days ago; 12 commits/90d

### Code health
Three co-equal signals: defect risk 6.03/10 avg, hotspot health 4.85/10 (stable), worst `packages/mcp/src/session.ts` at 1.0/10 · maintainability 8.46/10 · performance risk 39 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- `native/SimViewCore/Sources/SimViewAXShim/SimViewAXShim.m` — prior defect — impact −1.4
- `packages/client/src/protocol.ts` — churn risk — impact −1.1
- `packages/client/src/client.ts` — low cohesion (SimViewClient) — impact −1.0

### Standing decisions (ask `get_why` before diverging)
- One shared detached backend per device identity, keyed on protocol and binary hash — A SHA-256-derived instance ID stops a new packaged binary attaching to an incompatible old backend, 
...
- Newest-frame-wins backpressure across independent bounded queues — A slow viewer must never accumulate an unbounded video backlog and capture work must never delay inp
