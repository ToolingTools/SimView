...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-08 (commit 91b57a1). Confidence: 98%.
...
### Key modules
- `packages/contracts/src` — Browser-safe schemas, wire types, protocol constants, and shared payload contracts.
- `native/SimViewCore/Sources/SimViewCore` — Native iOS Simulator services, authenticated transport, capture, accessibility, and input handling.
...
- `native/SimViewCore` — Swift package containing the native executable, compatibility shim, and XCTest suite.
...
- `native/SimViewAndroid/src/dev/simview/agent` — Android agent entry point, device session, screen streaming, accessibility, and input handling.
...
- `packages/mcp/src/server.ts` — 3 bug fixes, last fix 2 days ago; 15 commits/90d
- `packages/app/src/index.tsx` — 3 bug fixes, last fix 7 days ago; 17 commits/90d
- `tests/mcp-tools.test.ts` — 2 bug fixes, last fix 2 days ago; 11 commits/90d
- `native/SimViewCore/Sources/SimViewAXShim/SimViewAXShim.m` — 2 bug fixes, last fix 8 days ago; 4 commits/90d
- `packages/mcp/src/session.ts` — 2 bug fixes, last fix 8 days ago; 10 commits/90d

### Code health
Three co-equal signals: defect risk 6.29/10 avg, hotspot health 4.07/10 (stable), worst `packages/app/src/helpers.ts` at 1.0/10 · maintainability 8.49/10 · performance risk 39 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `packages/mcp/src/server.ts` — untested hotspot — impact −2.0
- `packages/mcp/src/session.ts` — untested hotspot — impact −2.0
