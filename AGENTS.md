...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-09 (commit 43ac746). Confidence: 98%.
...
- `native/SimViewCore/Sources/SimViewAXShim/SimViewAXShim.m` — 3 bug fixes, last fix today; 10 commits/90d
- `packages/mcp/src/server.ts` — 3 bug fixes, last fix 3 days ago; 19 commits/90d
- `packages/app/src/index.tsx` — 3 bug fixes, last fix 8 days ago; 18 commits/90d
- `tests/mcp-tools.test.ts` — 2 bug fixes, last fix 3 days ago; 15 commits/90d
- `packages/mcp/src/session.ts` — 2 bug fixes, last fix 8 days ago; 13 commits/90d

### Code health
Three co-equal signals: defect risk 5.58/10 avg, hotspot health 4.49/10 (stable), worst `packages/mcp/src/server.ts` at 1.0/10 · maintainability 8.43/10 · performance risk 39 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- Embedded preview transports over the MCP host bridge, not loopback HTTP — It is the only transport that works in every MCP host without asking the user to trust a certificate
