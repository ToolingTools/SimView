...
final class AgentSession {
    ...
    try {
      while (true) {
        ...
        try {
          ...
          if (command == START_CAPTURE || command == REQUEST_KEYFRAME) throw error;
        }
      }
    } finally {
      if (inputDispatcher != null) inputDispatcher.cancelActiveTouch();
      stopCapture();
    }
  }
  ...
  private ScreenStreamer requireStreamer() {
    if (streamer == null) throw new IllegalStateException("capture has not started");
    return streamer;
  }

  private InputDispatcher inputDispatcher() {
    if (inputDispatcher == null) inputDispatcher = new InputDispatcher();
